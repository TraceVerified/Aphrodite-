...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-muted/30 text-foreground pb-16 lg:pb-0">
      {/* Desktop Sidebar */}
      ...
      {/* Mobile Bottom Nav */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 flex h-16 items-center justify-around border-t border-border bg-card px-2 lg:hidden">
        {NAV.map((item) => {
          const active = item.exact
            ? pathname === item.href
            : pathname.startsWith(item.href);
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex flex-col items-center justify-center gap-0.5 rounded-md px-2 py-1 text-xs font-medium transition-colors",
                active
                  ? "text-primary"
